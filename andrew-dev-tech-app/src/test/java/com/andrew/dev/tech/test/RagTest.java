...
public class RagTest {
    ...
    public void upload() {
        ...
        List<Document> documentSplit = tokenTextSplitter.apply(documents);

        documents.forEach(doc -> doc.getMetadata().put("knowledge", "知识库名称"));
        documentSplit.forEach(doc -> doc.getMetadata().put("knowledge", "知识库名称"));

        pgVectorStore.accept(documentSplit);
    ...
    public void chat() {
        ...
        ChatResponse chatResponse = chatClient.call(
                new Prompt(
                        messages, OllamaOptions.create().withModel("deepseek-r1:1.5b")
                )
        );
